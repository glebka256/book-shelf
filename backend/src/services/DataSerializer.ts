...
export class DataSerializer {
    private associates: SubjectAssociates[];

    constructor() {
        this.associates = Object.entries(associations).map(([key, value]) => ({
            subject: {
                name: key,
    ...
    getParsingSubjects(type: ScrapingTypes ): string[] {
        if (type === 'all') {
    ...
    getAllAsociates(): SubjectAssociates[] {
        return this.associates;
    }

    getAssociations(name: string): SubjectAssociates {
